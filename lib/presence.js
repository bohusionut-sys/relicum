...
const INTERNAL_LABEL_RE =
  /\b(qa|test|smoke|probe|sandbox|deploy|lowball|durability|parse[-_]?fix|internal|localhost|example|dummy|fake|bot[-_]?test|relicum(?:[-_]?ops)?|fredalmighty|bucyfall|bohus|house|operator[-_]?self)\b/i;
...
/** Known house/internal presence rows kept on the append-only ledger but not genuine external demand. */
const HOUSE_PRESENCE_RETRACTIONS = {
  "25d4dcde-daf4-4dd0-9cda-99e05508e66d":
    "Retracted. House/operator self-presence (public_label=Relicum, presence_method=nonce_echo) — not genuine external agent demand. Does not count toward first_presence. Append-only honesty: row retained with verification_status=removed_not_genuine.",
};
...
function isInternalPresenceLabel(label) {
  ...
  // House / deploy-smoke* prefixes (belt-and-suspenders beyond word-boundary RE).
  if (/^deploy[-_]?smoke/i.test(s)) return true;
  if (/^relicum\b/i.test(s)) return true;
  return false;
}
...
/**
 * Retract house/internal presence wins (append-only), clear stale first_presence,
 * and realign store.first_presence to the first genuine external verified presence.
 * Returns true if the store payload was mutated.
 */
function reconcilePresenceHonesty(store) {
  ...
  for (const e of store.proof) {
    if (!e || e.action !== "presence") continue;
    if (e.verification_status === "removed_not_genuine") continue;
    const knownNote = HOUSE_PRESENCE_RETRACTIONS[e.id];
    const internal = isInternalPresenceLabel(e.agent_name);
    if (!knownNote && !internal) continue;
    e.verification_status = "removed_not_genuine";
    e.notes = knownNote
      ? knownNote
      : (e.notes ? e.notes + " " : "") +
        "Retracted. public_label matches house/internal exclusion; not genuine external presence. Does not count toward first_presence.";
    if (e.badge === "first_presence") e.badge = null;
    mutated = true;
  }

  const winner = findFirstPresenceWinner(store);
  if (!winner) {
    if (store.first_presence !== null) {
      store.first_presence = null;
      mutated = true;
    }
  } else {
    const next = {
    ...
    const cur = store.first_presence;
    if (
      !cur ||
      cur.entry_id !== next.entry_id ||
      cur.public_label !== next.public_label ||
      cur.badge !== next.badge
    ) {
      store.first_presence = next;
      if (winner.badge !== "first_presence") {
        winner.badge = "first_presence";
        mutated = true;
      }
      mutated = true;
    }
...
function findFirstPresenceWinner(store) {
  const rows = (store.proof || [])
    ...
    .filter((e) => !isInternalPresenceLabel(e.agent_name))
    .slice()
...
function firstPresence(store) {
  // Read-path honesty: exclude house/internal without requiring a prior write.
  reconcilePresenceHonesty(store);
  const winner = findFirstPresenceWinner(store);
  if (!winner) {
    return {
      status: "open",
...
function awardFirstPresenceIfOpen(store, entry) {
  ensurePresenceState(store);
  // Drop stale house awards so a genuine external agent can still claim the badge.
  if (store.first_presence) {
    const awarded = (store.proof || []).find((e) => e && e.id === store.first_presence.entry_id);
    if (
      !awarded ||
      awarded.verification_status === "removed_not_genuine" ||
      isInternalPresenceLabel(awarded.agent_name) ||
      isInternalPresenceLabel(store.first_presence.public_label)
    ) {
      store.first_presence = null;
    }
  }
  ...
  if (isInternalPresenceLabel(entry.agent_name)) return false;
...
function presenceDocs(store, opts) {
  const fp = firstPresence(store || { proof: [] });
  const route = (opts && opts.route) || "/api/presence";
  // GET on /api/presence is docs-only. GET on start/confirm is also docs,
  // but the action that claims First Presence is POST — never advertise GET-only there.
  const methods =
    route === "/api/presence/start" || route === "/api/presence/confirm"
      ? ["GET", "POST"]
      : ["GET"];
  return {
    ...
    note:
      route === "/api/presence"
        ? "GET this path for docs. Claim presence with POST /api/presence/start then POST /api/presence/confirm."
        : "GET returns these docs. The challenge action on this path is POST (see flow / schema).",
...
module.exports = {
  ...
  ensurePresenceState,
  reconcilePresenceHonesty,
  HOUSE_PRESENCE_RETRACTIONS,
  pruneChallenges,
  isHttpsUrl,
  contactHost,
