...
const HONESTY_NOTE =
  "Entries with verification_status=removed_not_genuine are internal demo, sandbox, QA probe, or house/operator self-writes (original build curls, durability/parse probes, and house-label presence). They stay on the append-only ledger and do not count toward standing_high_gbp, reserve_met, next_minimum_gbp, or first_presence. They were not real external agent bids or genuine external presence.";
...
async function loadStore() {
  ...
  // Retract house/internal first_presence wins; reopen badge for genuine external agents.
  try {
    const { reconcilePresenceHonesty } = require("./presence");
    if (store && reconcilePresenceHonesty(store)) dirty = true;
  } catch (err) {
    // Presence module optional at boot; never block store load.
  }
  if (dirty) {
